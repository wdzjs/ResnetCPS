...
def main(modelname,stat_time):
    ...
    #added by yxy
    inputsize= tuple((1,)) + data_config['input_size']
    if stat_time=='statlist':
        modelstat = stat(model, data_config['input_size'])
        return modelstat
    ...
    if args.rank == 0:
        ...
        #with open(os.path.join(output_dir, 'args.yaml'), 'w') as f:
            #f.write(args_text)
        #added by yxy
        #with open(os.path.join(output_dir, 'torchstat.txt'), 'w') as f:
            #f.write(modelstat)
    #added by yxy
    if stat_time=='time':
        time= calcTime(model, inputsize)
        return time
...
def calcTime(model,size):
    ...
    dummy_input = torch.rand(size).cuda()
...
if __name__ == '__main__':
    ...
    ratio = [0, 16, 8, 4, 2]
    with open(os.path.join(r'E:\results\cross-layer', 'stat.txt'), 'a') as f:
        for i in modelname:
            #name = i
            name = i + 'PS'
            statlist = main(name, 'statlist')
            #r = i+',original,'+statlist
            #r = i+',ratio=16,'+statlist
            #r = i+',ratio=8,'+statlist
            #r = i+',ratio=4,'+statlist
            r = i+',ratio=2,'+statlist
            f.write(r)
            
    with open(os.path.join(r'E:\results\cross-layer', 'time.txt'), 'a') as f:
        for i in modelname:
            name = i
            #name = i + 'PS'
            line = main(name, 'time')
    ...
    with open(os.path.join(r'E:\results\cross-layer', 'modelcmp.txt'), 'w') as f:
        for i in modelname:
            name = i
            try:
                statlist = main(name, 'statlist')
